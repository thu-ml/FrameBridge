...
def get_args():
    ...
    parser.add_argument(
        "--on_fly_loading",
        action="store_true",
        help="load and process datasets when training"
    )
    parser.add_argument(
        "--video_folder",
        type=str,
        default=None,
        help="folder to fine videos"
    )
    parser.add_argument(
        "--text_dropout",
        type=float,
        default=0.1,
        help="Probability to drop text conditions.",
    )
    parser.add_argument(
        "--sft_training",
        action="store_true",
        help="Use SFT instead of LoRA",
    )
    parser.add_argument(
        "--validation_steps",
        type=int,
    ...
    parser.add_argument(
        "--use_bridge",
        action="store_true",
        help="Whether to use FrameBridge"
    )
    parser.add_argument(
        "--concat_type",
        type=str,
        default="channel",
        help="use channel or temporal concatenation"
    )
    parser.add_argument(
        "--learned_pos_embed",
        action="store_true",
        help="Whether to learn positional embeddings"
    )
    parser.add_argument(
        "--noisy_cond",
        action="store_true",
        help="Add noise to condition image"
    )
    parser.add_argument(
        "--noisy_prior",
        action="store_true",
        help="Add noise to bridge prior"
    )
    parser.add_argument(
        "--non_zero_channel_padding",
        action="store_true",
        help="Concatenate replicated image along channel axis"
    )
    parser.add_argument(
        "--only_first_frame",
        action="store_true",
        help="(Debug only) Train the model to generate the static first frame"
    )
    parser.add_argument(
        "--use_tf32",
        action="store_true",
        help="Enable tf32 precision for training."
    )
...
def log_validation(
    ...
    concat_type: str = "channel",
    use_bridge=False,
    bridge_scheduler=None,
    non_zero_channel_padding=False,
):
    ...
    pipeline_args.update({
        ...
        "bridge_scheduler": bridge_scheduler,
        "non_zero_channel_padding": non_zero_channel_padding,
    })
    ...
    # print("tracker: ", accelerator.trackers)
    ...
    for i, video in enumerate(videos):
        ...
        filename = os.path.join(args.output_dir, "valid", f"validation_video_epoch{epoch}_step{step}_{i}_{prompt}.mp4")
        export_to_video(video, filename, fps=8)
        video_filenames.append(filename)
...
def loss_diff_channel_concat_unified(model_context, data_context, config):
    autoencoder = model_context["autoencoder"]
    transformer = model_context["transformer"]
    scheduler = model_context["scheduler"]
    bridge_scheduler = model_context["bridge_scheduler"]
    device = model_context["device"]
    dtype = model_context["dtype"]
    logvar_model = model_context["logvar_model"]

    with torch.no_grad():  # no autocast because T5 overflows at softmax under pure fp16
        target_ls = data_context["target_frame_ls"].to(device, dtype=dtype, non_blocking=True)  # B, C, T, H, W
        bridge_prior = data_context["bridge_prior"].to(device, dtype=dtype, non_blocking=True)
        bridge_prior_no_noise = data_context["bridge_prior_no_noise"].to(device, dtype=dtype, non_blocking=True)
        B, C, T, H, W = target_ls.shape

        X0 = autoencoder.encode(target_ls).latent_dist.mode() * autoencoder.config.scaling_factor
        bridge_prior = autoencoder.encode(bridge_prior).latent_dist.mode() * autoencoder.config.scaling_factor
        bridge_prior_no_noise = autoencoder.encode(bridge_prior_no_noise).latent_dist.mode() * autoencoder.config.scaling_factor

        X0 = rearrange(X0, "b c t h w -> b t c h w")
        bridge_prior = rearrange(bridge_prior, "b c t h w -> b t c h w")
        bridge_prior_no_noise = rearrange(bridge_prior_no_noise, "b c t h w -> b t c h w")
        B, T_L, C_L, H_L, W_L = X0.shape
        text_embedding = data_context["prompt_embeds"]

        source_ls = data_context["source_frame_ls"].to(device, dtype=dtype, non_blocking=True)  # B, C, T, H, W
        X_cond = autoencoder.encode(source_ls).latent_dist.mode() * autoencoder.config.scaling_factor
        X_cond = rearrange(X_cond, "b c t h w -> b t c h w") 
    ...
    image_rotary_emb = (
        prepare_rotary_positional_embeddings(
            ...
            width=config["width"],
            num_frames=T_L,
            vae_scale_factor_spatial=config["vae_scale_factor_spatial"],
    ...
    with torch.autocast("cuda", dtype=dtype):
        ...
        if model_context["use_bridge"]:
            Xt = bridge_scheduler.add_noise(X0, noise, timesteps, bridge_prior)
            aligned_Xt, aligned_timesteps, aligned_Xt_from_ori = model_context["bridge_scheduler"].align_marginal_and_timesteps(Xt, timesteps, bridge_prior, X0, noise)
        else:
            aligned_Xt, aligned_timesteps = Xt, timesteps
        
        if config["concat_type"] == "channel":
            hidden_states = torch.cat([aligned_Xt, X_cond], dim=2)
        else:
            assert config["concat_type"] == "temporal"
            hidden_states = torch.cat([X_cond[:, :3, ...], aligned_Xt[:, 3: ,...]], dim=1)

        pred = transformer(
            hidden_states=hidden_states,
            encoder_hidden_states=text_embedding,
            timestep=aligned_timesteps,
            image_rotary_emb=image_rotary_emb,
        ).sample

        if scheduler.config.prediction_type == "v_prediction":
            if model_context["use_bridge"]:
                target = bridge_scheduler.get_v_target(X0, noise, timesteps, Xt)
            else:
                target = scheduler.get_velocity(X0, noise, timesteps)
        elif scheduler.config.prediction_type == "epsilon":
            assert model_context["use_bridge"] is False
            target = noise
        else:
            raise ValueError(f"Invalid prediction type: {scheduler.config.prediction_type}")

        if config["concat_type"] == "temporal":
            target = target[:, 3:, ...]
            pred = pred[:, 3:, ...]
        
        weights = 1.
        loss = torch.mean(weights * ((target - pred) ** 2.))
...
def main(args):
    ...
    if args.learned_pos_embed:
        transformer.patch_embed.pos_embedding = torch.nn.Parameter(transformer.patch_embed.pos_embedding)
    ...
    if args.use_bridge:
        bridge_scheduler = BridgeScheduler(scheduler.alphas_cumprod ** 0.5, (1 - scheduler.alphas_cumprod) ** 0.5)
    else:
        bridge_scheduler = None
    ...
    assert weight_dtype == torch.float32, "Currently, we find that training with fp16 or bf16 may lead to unsatisfactory results."
    ...
    if not args.sft_training:
        ...
        for param in transformer.patch_embed.proj.parameters():
    ...
    def load_model_hook(models, input_dir):
        if not args.sft_training:
            ...
            transformer_.load_state_dict(load_model.state_dict(), assign=init_under_meta, strict=False)
            del load_model

            # Then, load LoRA parameters
    ...
    def encode_video(video, bar=None):
        bar.update(1)
    ...
    def construct_source_and_target_frames(video, noisy_cond=False, noisy_prior=False, non_zero_channel_padding=False, concat_type="channel", only_first_frame=False):
        video = video.to(accelerator.device, dtype=vae.dtype)
        if only_first_frame:
            video = video[:, :, :1].repeat(1, 1, video.shape[2], 1, 1)
        
        # construct image condition
        image = video[:, :, :1].clone()
        if noisy_cond:
            image_noise_sigma = torch.normal(mean=-3.0, std=0.5, size=(image.shape[0],), device=image.device)
            image_noise_sigma = torch.exp(image_noise_sigma).to(dtype=image.dtype)
            image = image + torch.randn_like(image) * image_noise_sigma[:, None, None, None, None]
        
        if args.concat_type == "channel":
            assert non_zero_channel_padding
            cond = image.repeat(1, 1, video.shape[2], 1, 1)
            bridge_prior = video[:, :, :1].clone().repeat(1, 1, video.shape[2], 1, 1)
        else:
            assert args.concat_type == "temporal"
            cond = image.repeat(1, 1, 8 + video.shape[2], 1, 1)
            bridge_prior = video[:, :, :1].clone().repeat(1, 1, 8 + video.shape[2], 1, 1)
            video = torch.concat([image.repeat(1, 1, 9, 1, 1), video[:, :, 1:, :, :]], dim=2)

        bridge_prior_no_noise = bridge_prior.clone()
        if noisy_prior:
            prior_noise_sigma = torch.normal(mean=-3.0, std=0.5, size=(bridge_prior.shape[0],), device=bridge_prior.device)
            prior_noise_sigma = torch.exp(prior_noise_sigma).to(dtype=image.dtype)
            bridge_prior = bridge_prior + torch.randn_like(bridge_prior) * prior_noise_sigma[:, None, None, None, None]

        return cond, video, bridge_prior, bridge_prior_no_noise
    
    # Dataset and DataLoader
    if args.on_fly_loading:
        train_dataset = WebVid(
            args.instance_data_root,
            args.video_folder,
            subsample=None,
            video_length=args.max_num_frames,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                source_frame_ls, target_frame_ls, bridge_prior, bridge_prior_no_noise = construct_source_and_target_frames(
                    batch["video"],
                    noisy_cond=args.noisy_cond,
                    noisy_prior=args.noisy_prior,
                    non_zero_channel_padding=args.non_zero_channel_padding,
                    concat_type=args.concat_type,
                    only_first_frame=args.only_first_frame
                )

                prompts = batch["caption"]
                mask = torch.bernoulli(torch.ones([len(prompts)]) * args.text_dropout)
                ...
                model_context = {
                    ...
                    "bridge_scheduler": bridge_scheduler,
                    "logvar_model": logvar_model
                }
                data_context = {
                    "source_frame_ls": source_frame_ls,
                    "target_frame_ls": target_frame_ls,
                    "bridge_prior": bridge_prior,
                    "bridge_prior_no_noise": bridge_prior_no_noise,
                ...
                config = {
                    ...
                    "height": args.height,
                    "width": args.width,
                    "use_rotary_positional_embeddings": model_config.use_rotary_positional_embeddings,
                    "concat_type": args.concat_type,
                }
                loss = loss_diff_channel_concat_unified(model_context, data_context, config)["loss"]

                data_context.update(bridge_prior=bridge_prior_no_noise)
