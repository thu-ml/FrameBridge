...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y):
        B, N, C = x.shape
        q = self.wq(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
        k = self.wk(y).reshape(B, y.shape[1], self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
        v = self.wv(y).reshape(B, y.shape[1], self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, c, y=None):
        ...
        if self.cross_attn:
            attn_output = self.attn(modulate(self.norm1(x), shift_msa, scale_msa), y)
        else:
...
class Latte(nn.Module):
    ...
    def __init__(
        ...
        if spatial_only is True:
            self.out_channels *= dct_coef_num
        self.patch_size = patch_size
        self.num_heads = num_heads
        self.extras = extras
        print("use_pretrained_nwarp: ", use_pretrained_nwarp)
        if use_pretrained_nwarp:
            in_channels *= 2
            self.in_channels *= 2
            '''
            in_channels += 1
        ...
        if dct_domain:
            num_frames = dct_coef_num
            
        print("condition_type:", condition_type)
        if condition_type == "concat_pre":
            num_frames += 1
        elif condition_type in ["cross_attention", "adaln"]:
            self.cond_embedder = PatchEmbed(input_size, patch_size, in_channels, hidden_size, bias=True)
        else:
            assert condition_type is None
        self.num_frames = num_frames

        self.x_embedder = PatchEmbed(input_size, patch_size, in_channels, hidden_size, bias=True)
        if model_stage == "nwarp":
        ...
        if condition_type == "cross_attention":
            ...
            for _ in range(depth // 3):
                modulelist.append(TransformerBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, attention_mode=attention_mode, cross_attn=True)) # conditional corss attn
                modulelist.append(TransformerBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, attention_mode=attention_mode)) # spatial attn
                modulelist.append(TransformerBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, attention_mode=attention_mode)) # temporal attn
        else:
            modulelist = [
                TransformerBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, attention_mode=attention_mode) for _ in range(depth)
            ]
        self.blocks = nn.ModuleList(modulelist)
        ...
        if finetune_spatial_only is True:
            for i in range(depth // 2):
                modulelist[2 * i + 1].requires_grad_(False)
    ...
    def forward(self, 
        ...
        cond_frame: (N, C, H, W) tensor of conditional frame
        """
        if use_fp16:
            x = x.to(dtype=torch.float16)

        batches = x.shape[0]
        if self.spatial_only is False:
        ...
        if self.condition_type in ["cross_attention", "adaln"]:
            cond_embedding = self.cond_embedder(condition_frame) + self.pos_embed
            cond_embedding_spatial = repeat(cond_embedding, 'n d -> (n c) d', c=self.temp_embed.shape[1])
            cond_embedding_temp = repeat(cond_embedding, 'n d -> (n c) d', c=self.pos_embed.shape[1])
            cond_embedding_ca = cond_embedding.repeat(self.num_frames, 1, 1)

            
        for i in range(0, len(self.blocks), num_transformer_blocks):
            if self.extras == 2:
            ...
            if self.condition_type == "cross_attention":
                condition_block, spatial_block, temp_block = self.blocks[i:i+3]
                x = condition_block(x, c, cond_embedding_ca)
            elif self.spatial_only == True:
    ...
    def forward_with_cfg(self, x, t, y=None, cfg_scale=7.0, use_fp16=False, text_embedding=None, condition_frame=None):
        """
        ...
        model_out = self.forward(combined, t, y=y, use_fp16=use_fp16, text_embedding=text_embedding, condition_frame=condition_frame)
...
class Latte_nwarp(nn.Module):
    def __init__(
        self,
        nwarp_supervision_type="regression",
        **kwargs
    ):
        super().__init__()
        print("nwarp_supervision_type: ", nwarp_supervision_type)
        
        # Initialize diffusion model
        if nwarp_supervision_type in ["denoising", "combination"]:
            kwargs["model_stage"] = "diffusion"
            kwargs["use_pretrained_nwarp"] = True
            self.diffusion_model = Latte(**kwargs)
        
        # Initialize nwarp model  
        kwargs["condition_type"] = None
        kwargs["model_stage"] = "nwarp"
        kwargs["use_pretrained_nwarp"] = False
        if kwargs["dct_domain"] is True and kwargs["dct_coef_num"] <= 2:
            kwargs["spatial_only"] = True
        self.nwarp_model = Latte(**kwargs)
        self.nwarp_supervision_type = nwarp_supervision_type
    ...
    def forward(self, 
            ...
            x_nwarp,
            condition_frame,
            conditioned_input_fn,
            y=None, 
            text_embedding=None, 
            use_fp16=False):
        
        coarse_video = self.nwarp_model(x_nwarp, None, y, text_embedding, use_fp16)
        if self.nwarp_supervision_type in ["denoising", "combination"]:
            x_diffusion_conditioned = conditioned_input_fn(x_diffusion, condition_frame, coarse_video)
            diffusion_output = self.diffusion_model(x_diffusion_conditioned, t, y, text_embedding, use_fp16)
        else:
...
def Latte_nwarp_S_2(**kwargs):
    print(kwargs)
    return Latte_nwarp(depth=12, hidden_size=384, patch_size=2, num_heads=6, **kwargs)
...
Latte_models = {
    ...
    'Latte-nwarp-S/2': Latte_nwarp_S_2,
}
