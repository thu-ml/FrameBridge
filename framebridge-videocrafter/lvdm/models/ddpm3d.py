...
from lvdm.models.samplers.bridge_first_order import BridgeFirstOrderSampler
...
def inference_load_model_checkpoint(model, ckpt):
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        super().__init__()
        assert parameterization in ["eps", "x0", "v", "eps_psi", "v_psi", "eps_psi_new"], 'currently only supporting "eps" and "x0" and "v"'
        ...
        self.model = DiffusionWrapper(unet_config, conditioning_key, temporal_concat=temporal_concat)
    ...
    def snr_align(self, snr, sl, sr, tl, tr):
        ratio = (snr - sl) / (sr - sl)
        return tl + ratio * (tr - tl)

    def rescale_align(self, aligned_t, sl, sr, tl, tr):
        ratio = (aligned_t - tl) / (tr - tl)
        return sl + ratio * (sr - sl)
        
    def register_schedule_bridge(self, bridge_beta_min, bridge_beta_max, timesteps=1000):
        ...
        sigma_1 = ((bridge_beta_min + bridge_beta_max) / 2.) ** 0.5
        bridge_sigmas = (((0.5 * (bridge_beta_max - bridge_beta_min) * bridge_t * bridge_t + bridge_beta_min * bridge_t)) ** 0.5)
        ...
        self.register_buffer('bridge_coef_x0', to_torch(bridge_coef_x0))
        self.register_buffer('bridge_coef_x1', to_torch(bridge_coef_x1))
        self.register_buffer('bridge_sqrt_var', to_torch(bridge_sqrt_var))
        self.register_buffer('bridge_alphas', to_torch(bridge_alphas))
        self.register_buffer('bridge_alphas_bar', to_torch(bridge_alphas_bar))
        self.register_buffer('bridge_sigmas', to_torch(bridge_sigmas))
        self.register_buffer('bridge_sigmas_bar', to_torch(bridge_sigmas_bar))
        
        # SNR Alignment
        bridge_snr = self.bridge_coef_x0 / self.bridge_sqrt_var
        if self.use_dynamic_rescale:
            original_diffusion_rescale = np.ones(bridge_snr.shape)
            diffusion_snr = self.sqrt_alphas_cumprod * self.scale_arr[:self.timesteps] / self.sqrt_one_minus_alphas_cumprod
        else:
            diffusion_snr = self.sqrt_alphas_cumprod / self.sqrt_one_minus_alphas_cumprod
        ...
        for i in range(len(bridge_snr)):
            if bridge_snr[i] > diffusion_snr[1]:
                if self.use_dynamic_rescale:
                    original_diffusion_rescale[i] = 1.
                bridge_aligned_t[i] = self.snr_align(bridge_snr[i], diffusion_snr[0] + bridge_snr[0], diffusion_snr[1], 0, 1)
            else:
                while cur < len(diffusion_snr) - 1 and bridge_snr[i] < diffusion_snr[cur + 1]:
                    cur += 1
                if bridge_snr[i] < diffusion_snr[-1]:
                    bridge_aligned_t[i] = self.snr_align(bridge_snr[i], diffusion_snr[-1], 0., len(diffusion_snr) - 1, len(diffusion_snr))
                else:
                    bridge_aligned_t[i] = self.snr_align(bridge_snr[i], diffusion_snr[cur], diffusion_snr[cur + 1], cur, cur + 1)
                if self.use_dynamic_rescale:
                    original_diffusion_rescale[i] = self.rescale_align(bridge_aligned_t[i], self.scale_arr[cur], self.scale_arr[cur + 1], cur, cur + 1)
        self.register_buffer('bridge_aligned_t', to_torch(bridge_aligned_t))
        if self.use_dynamic_rescale:
            diffusion_scale = ((self.sqrt_alphas_cumprod * self.scale_arr[:self.timesteps]) ** 2. + self.sqrt_one_minus_alphas_cumprod ** 2.) ** 0.5
            self.register_buffer("original_diffusion_rescale", to_torch(original_diffusion_rescale))
            self.register_buffer('diffusion_scale', diffusion_scale)
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        if self.parameterization in ["eps", "eps_psi", "eps_psi_new"]:
            lvlb_weights = self.betas ** 2 / (
                        2 * self.posterior_variance * to_torch(alphas) * (1 - self.alphas_cumprod))
        elif self.parameterization == "x0":
            lvlb_weights = 0.5 * np.sqrt(torch.Tensor(alphas_cumprod)) / (2. * 1 - torch.Tensor(alphas_cumprod))
        elif self.parameterization in ["v", "v_psi"]:
    ...
    def get_eps_psi(self, x_start, x_noisy, t):
        alpha_t = extract_into_tensor(self.bridge_alphas, t, x_start.shape)
        sigma_t = extract_into_tensor(self.bridge_sigmas, t, x_start.shape)
        return (x_noisy - alpha_t * x_start) / (alpha_t * sigma_t)

    def get_eps_psi_new(self, x_start, x_noisy, t, bridge_prior):
        bridge_a_t, bridge_b_t, bridge_c_t =  (extract_into_tensor(self.bridge_coef_x0, t, x_noisy.shape), 
                                                extract_into_tensor(self.bridge_coef_x1, t, x_noisy.shape),
                                                extract_into_tensor(self.bridge_sqrt_var, t, x_noisy.shape))
        alpha_t = extract_into_tensor(self.bridge_alphas, t, x_start.shape)
        sigma_t = extract_into_tensor(self.bridge_sigmas, t, x_start.shape)
        coef = (bridge_c_t / (bridge_a_t * sigma_t)) ** 2.
        return (x_noisy - bridge_b_t * bridge_prior - bridge_a_t * x_start) / bridge_c_t, coef
    
    def get_v_psi(self, x_start, x_noisy, noise, t):
        bridge_a_t, bridge_b_t, bridge_c_t =  (extract_into_tensor(self.bridge_coef_x0, t, x_noisy.shape), 
                                                extract_into_tensor(self.bridge_coef_x1, t, x_noisy.shape),
                                                extract_into_tensor(self.bridge_sqrt_var, t, x_noisy.shape))
        original_scale = extract_into_tensor(self.original_diffusion_rescale, t, x_noisy.shape)
        return (bridge_a_t / ((bridge_a_t ** 2. + bridge_c_t ** 2.) ** 0.5) * noise
                - bridge_c_t / ((bridge_a_t ** 2. + bridge_c_t ** 2.) ** 0.5) * original_scale * x_start)
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        elif self.parameterization == "eps_psi":
            target = self.get_eps_psi(x_start, x_noisy, t)
        elif self.parameterization == "v_psi":
            target = self.get_v_psi(x_start, x_noisy, noise, t)
        else:
    ...
    def training_step(self, batch, batch_idx):
        loss, loss_dict = self.shared_step(batch, training=True)
    ...
    def validation_step(self, batch, batch_idx):
        _, loss_dict_no_ema = self.shared_step(batch, Training=False)
        with self.ema_scope():
            _, loss_dict_ema = self.shared_step(batch, Training=False)
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        if use_bridge:
            self.register_schedule_bridge(bridge_beta_min=bridge_beta_min, bridge_beta_max=bridge_beta_max, timesteps=self.timesteps)

        self.use_cil = use_cil
        self.cil_beta_max = cil_beta_max
        self.use_svd_noise = use_svd_noise
    ...
    def forward(self, x, c, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.use_dynamic_rescale and (not self.use_bridge):
    ...
    def apply_model(self, x_noisy, t, cond, **kwargs):
        ...
        bridge_a_t, bridge_b_t, bridge_c_t =  (extract_into_tensor(self.bridge_coef_x0, t, x_noisy.shape), 
                                                ...
                                                extract_into_tensor(self.bridge_sqrt_var, t, x_noisy.shape))
        if self.use_dynamic_rescale:
            diffusion_scale_t = extract_into_tensor(self.diffusion_scale, t, x_noisy.shape)
        else:
            diffusion_scale_t = 1.
        bridge_prior = kwargs.pop("bridge_prior")
        aligned_x_noisy = diffusion_scale_t * (x_noisy - bridge_b_t * bridge_prior) / (bridge_a_t ** 2. + bridge_c_t ** 2.) ** 0.5
        aligned_t = extract_into_tensor(self.bridge_aligned_t, t, t.shape)
        
        x_recon = self.model(aligned_x_noisy, aligned_t, **cond, **kwargs)
    ...
    def construct_prior(self, x, cond):
        first_frame = cond["c_concat"][0]
        return first_frame
        
    def q_sample(self, x_start, t, noise=None, x_prior=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        if self.use_bridge is False:
            return (extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start +
                    extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * noise)
        else:
            return (extract_into_tensor(self.bridge_coef_x0, t, x_start.shape) * x_start
                    + extract_into_tensor(self.bridge_coef_x1, t, x_start.shape) * x_prior
                    + extract_into_tensor(self.bridge_sqrt_var, t, x_start.shape) * noise)
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        if self.use_bridge:
            bridge_prior = self.construct_prior(x_start, cond)
            x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise, x_prior=bridge_prior)
        ...
        if self.parameterization == "x0":
            ...
            target = self.get_eps_psi(x_start, x_noisy, t)
        elif self.parameterization == "eps_psi_new":
            target, _ = self.get_eps_psi_new(x_start, x_noisy, t, bridge_prior)
        elif self.parameterization == "v_psi":
            target = self.get_v_psi(x_start, x_noisy, noise, t)
        else:
    ...
    def training_step(self, batch, batch_idx):
        loss, loss_dict = self.shared_step(batch, random_uncond=self.classifier_free_guidance, training=True)
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        if ddim:
            if self.use_bridge:
                ddim_sampler = BridgeFirstOrderSampler(self)
                x_T = self.construct_prior(None, cond)
            else:
                ddim_sampler = DDIMSampler(self)
                x_T = None
            shape = (self.channels, self.temporal_length, *self.image_size)
            samples, intermediates = ddim_sampler.sample(ddim_steps, batch_size, shape, cond, x_T=x_T, verbose=False, **kwargs)
...
class LatentVisualDiffusion(LatentDiffusion):
    def __init__(self, img_cond_stage_config, image_proj_stage_config, freeze_embedder=True, image_proj_model_trainable=True, use_image_proj_model=True, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.image_proj_model_trainable = image_proj_model_trainable
        self._init_embedder(img_cond_stage_config, freeze_embedder)
        if use_image_proj_model:
    ...
    def shared_step(self, batch, random_uncond, training, **kwargs):
        x, c, fs = self.get_batch_input(batch, random_uncond=random_uncond, return_fs=True, cil_noise=self.use_cil and training, svd_noise=self.use_svd_noise and training)
        kwargs.update({"fs": fs.long()})
        tmp = torch.zeros((1,), device=self.device, requires_grad=True)
        loss, loss_dict = self(x, c, **kwargs)
        return loss, loss_dict
    
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_cond_frame=False, return_original_input=False, cil_noise=False, svd_noise=False, **kwargs):
        ## x: b c t h w
        x = super().get_input(batch, self.first_stage_key)
        ## encode video frames x to z via a 2D encoder        
        z = self.encode_first_stage(x)
        ...
        if svd_noise:
            image_noise_sigma = torch.normal(mean=-3.0, std=0.5, size=(img.shape[0],), device=img.device)
            image_noise_sigma = torch.exp(image_noise_sigma).to(img.dtype)
            img = img + torch.randn_like(img) * image_noise_sigma.reshape(-1, 1, 1, 1)

        img = input_mask * img

        ## img: b c h w
        img_emb = self.embedder(img) ## b l c
        # For generating neural prior
        ...
        if cil_noise:
            cond['c_crossattn'] = [prompt_imb, img_emb]
            cond["cil_beta_max"] = self.cil_beta_max
        else:
            cond["c_crossattn"] = [torch.cat([prompt_imb, img_emb], dim=1)] ## concat in the seq_len dim
        cond["cil_noise"] = cil_noise
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key, temporal_concat=False):
        super().__init__()
        self.diffusion_model = instantiate_from_config(diff_model_config)
        self.conditioning_key = conditioning_key
        self.temporal_concat = temporal_concat

    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None,
                c_adm=None, s=None, mask=None, cil_noise=False, cil_beta_max=None, **kwargs):
        ...
        if self.conditioning_key is None:
            ...
            if cil_noise:
                cil_mu_t = 2 * ((t + 1.) / 1000.) ** 5. - 1
                # beta_s1 = torch.sigmoid(torch.randn_like(t) + cil_mu_t) * cil_beta_max
                beta_s2 = torch.sigmoid(torch.randn_like(t) + cil_mu_t) * cil_beta_max
                # c_concat[0] = c_concat[0] + beta_s1.reshape(-1, 1, 1, 1, 1) * torch.randn_like(c_concat[0])
                c_crossattn[1] = c_crossattn[1] + beta_s2.reshape(-1, 1, 1) * torch.randn_like(c_crossattn[1])
                c_crossattn = [torch.cat(c_crossattn, dim=1)]

            if self.temporal_concat:
                # concatenate along temporal axis
                xc = torch.cat([c_concat[0][:,:,:1,:,:]] + [x], dim=2)
            else:
                xc = torch.cat([x] + c_concat, dim=1)
