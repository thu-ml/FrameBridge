...
def state_dict_except_diffusion(model, pl_sd):
    new_pl_sd = OrderedDict()
    for k,v in pl_sd.items():
        if "diffusion" not in k:
            new_pl_sd[k] = v
    
    for k,v in model.state_dict().items():
        if "diffusion" in k or k not in pl_sd:
            new_pl_sd[k] = v
    return new_pl_sd

def load_diffusion_checkpoints(model, ckpt_path):
    pl_sd = torch.load(ckpt_path, map_location="cpu")["state_dict"]
    new_pl_sd = OrderedDict()
    model_dict = model.state_dict()
    for k,v in pl_sd.items():
        if "diffusion" in k and k in model_dict and v.shape == model_dict[k].shape:
            new_pl_sd[k] = v
    ...
    if check_config_attribute(model_cfg, "pretrained_checkpoint"):
        ...
        try:
            if 'state_dict' in pl_sd.keys():
                ...
                if check_config_attribute(model_cfg, "load_ckpt_except_diffusion"):
                    new_pl_sd = state_dict_except_diffusion(model, pl_sd["state_dict"])
                else:
                    new_pl_sd = pl_sd["state_dict"]

                if not check_config_attribute(model_cfg, "load_image_proj_model"):
                    load_pl_sd = new_pl_sd.copy()
                    for k,v in new_pl_sd.items():
                        if "image_proj_model" in k:
                            load_pl_sd[k] = model.state_dict()[k]
                    new_pl_sd = load_pl_sd.copy()
                    del load_pl_sd
                
                for key in new_pl_sd.keys():
                    if new_pl_sd[key].shape != model.state_dict()[key].shape:
                        new_pl_sd[key] = model.state_dict()[key].clone()
                ...
                if check_config_attribute(model_cfg, "load_ckpt_except_diffusion") and model_cfg.load_ckpt_except_diffusion:
                    new_pl_sd = state_dict_except_diffusion(model, pl_sd["state_dict"])
                else:
                    new_pl_sd = pl_sd["state_dict"]
...
def load_np_model(ckpt_path, config_path):
    ...
    model = inference_load_model_checkpoint(model, ckpt_path)
    model.eval()
    return model