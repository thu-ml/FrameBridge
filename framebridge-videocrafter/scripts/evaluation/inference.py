...
def load_data_prompts(data_dir, video_size=(256,256), video_frames=16, interp=False):
    ...
    for idx in range(n_samples):
        if interp:
            ...
            _, filename = os.path.split(file_list[idx][:-4])
...
def save_results_seperate(prompt, samples, filename, fakedir, fps=10, loop=False):
    ...
    for idx, video in enumerate(videos):
        ...
        for i in range(n):
            ...
            path = os.path.join(savedirs[idx].replace('samples', 'samples_separate'), filename + ".mp4")
            if grid.shape[0] == 1:
                grid = grid.repeat(16, 1, 1, 1)
...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., bridge_sampler=None, \
                        unconditional_guidance_scale=1.0, cfg_img=None, fs=None, text_input=False, multiple_cond_cfg=False, loop=False, interp=False, timestep_spacing='uniform', guidance_rescale=0.0, intermediate_results=False, prior_init=False, **kwargs):
    if model.use_bridge:
        if bridge_sampler == "bridge_sde":
            ddim_sampler = BridgeFirstOrderSampler(model)
        else:
            raise NotImplementedError(bridge_sampler)
    else:
    ...
    if prior_init is True:
        bridge_prior = img_cat_cond
    else:
        bridge_prior = None
    ...
    for _ in range(n_samples):
        ...
        if intermediate_results:
            for i in range(len(intermediate["x_inter"])):
                intermediate["x_inter"][i] = model.decode_first_stage(intermediate["x_inter"][i])
                inter_variants.append(intermediate["x_inter"][i])

        batch_variants.append(batch_images)
    ## variants, batch, c, t, h, w
    batch_variants = torch.stack(batch_variants)
    if intermediate_results:
        inter_variants = torch.stack(inter_variants)
        return batch_variants.permute(1, 0, 2, 3, 4, 5), inter_variants.permute(1, 0, 2, 3, 4, 5)
    else:
        return batch_variants.permute(1, 0, 2, 3, 4, 5), None
...
def run_inference(args, gpu_num, gpu_no):
    ...
    print(samples_split*gpu_no, samples_split*(gpu_no+1))
    if gpu_no == gpu_num - 1:
        indices = list(range(samples_split*gpu_no, num_samples))
        print(samples_split*gpu_no, num_samples)
    prompt_list_rank = [prompt_list[i] for i in indices]
    data_list_rank = [data_list[i] for i in indices]
    filename_list_rank = [filename_list[i] for i in indices]

    start = time.time()
    print("prompt_list_rank: ", len(prompt_list_rank))
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            prompts = prompt_list_rank[indice:min(len(prompt_list_rank), indice+args.bs)]
            videos = data_list_rank[indice:min(len(prompt_list_rank), indice+args.bs)]
            filenames = filename_list_rank[indice:min(len(prompt_list_rank), indice+args.bs)]
            if isinstance(videos, list):
                videos = torch.stack(videos, dim=0).cuda(gpu_no % 8)
            else:
                videos = videos.unsqueeze(0).cuda(gpu_no % 8)

            need_sample = False
            for nn in range(len(prompts)):
                file_path = os.path.join(fakedir.replace('samples', 'samples_separate'), filenames[nn] + ".mp4")
                if os.path.exists(file_path) is False:
                    need_sample = True
                    break
            if need_sample is False:
                continue

            noise_shape[0] = len(prompts)
            batch_samples, inter_samples = image_guided_synthesis(model, prompts, videos, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, args.bridge_sampler, \
                                args.unconditional_guidance_scale, args.cfg_img, args.frame_stride, args.text_input, args.multiple_cond_cfg, args.loop, args.interp, args.timestep_spacing, args.guidance_rescale, args.intermediate_results, args.prior_init)
...
def get_parser():
    ...
    ## Bridge-related sampling configurations
    parser.add_argument("--bridge_sampler", type=str, default=None, help="sampler for bridge process")
    parser.add_argument("--intermediate_results", action='store_true', default=False, help="save intermediate sampling results")
    parser.add_argument("--prior_init", action='store_true', default=False, help="initialize z_T with bridge prior")
    return parser
